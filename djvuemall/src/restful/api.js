...
// Axios.defaults.baseURL = 'https://www.luffycity.com/api/v1/courses/?sub_category';
...
// 测试跨域
export const testCoreApi = () => {
  return Axios.get('/demo/').then((res) => res.data);
};
...
export const allCategoryList = (categoryId) => {
  return Axios.get(`courses/?sub_category=${categoryId}`).then(
    (res) => res.data
  );
};
// 课程详情顶部数据
export const coursedetailtop = (courseid) => {
  return Axios.get(`coursedetailtop/?courseid=${courseid}`).then(
    (res) => res.data
  );
};
// 课程概述
export const coursedetail = (courseid) => {
  return Axios.get(`coursedetail/?courseid=${courseid}`).then(
    (res) => res.data
  );
};
