...
class SettlementView(APIView):
    ...
    def post(self, request):
        ...
        for course_id in course_list:
            ...
            # 3 构建数据结构
            # 3.1 获取用户的所有合法优惠券【如何判断】如果没有object_id 则是全局优惠券，因为课程的优惠券添加了object_id
            user_all_coupons = CouponRecord.objects.filter(
                account_id=user_id,  # 是这个用户的
                status=0,  # 优惠券是 未使用状态
                coupon__valid_begin_date__lte=now(),  # 开始时间小于等于当前时间
                coupon__valid_end_date__gte=now(),  # 开始时间大于等于当前时间
            ).all()
            ...
            for coupon_record in user_all_coupons:
                # 在 CouponRecord 的模型中，有外键关联
                # coupon = models.ForeignKey("Coupon", on_delete=models.CASCADE)
                coupon = coupon_record.coupon

                if coupon.object_id == course_id:  # 当前课程的优惠券
